const SUPERNOVA = {
    ...
    starGain() {
        let x = E(player.supernova.tree.includes("c")?1:0)
...
function updateSupernovaTemp() {
    ...
    tmp.supernova.maxlimit = E(1e20).pow(player.supernova.times.div(tmp.supernova.ml_fp).pow(1.25)).mul(1e85)
    tmp.supernova.bulk = player.stars.points.div(1e85).max(1).log(1e20).max(0).root(1.25).mul(tmp.supernova.ml_fp).add(1).floor()
